	...
	public function create_transaction()
	{
		...
		if(!empty($this->response) || $this->response != ''){
			...
			if(isset($this->response['status']) && $this->response['status'] == 'success'){
			    ...
			    if(@$data['payment_for'] == 'property'){
			        
    			    $payment_details = array("payment_for" => "property", "refID" => $referenceID, "bookingID" => $bookingID, "propertyID" => $propertyID, "verID" => $verificationID, "expiry" => $rentExpiry);
    			    
			    }else if(@$data['payment_for'] = 'wallet'){
			...
			if(!empty($response)){
				...
				if($response['status'] == 'success' && isset($response['data']['chargecode']) && ( $response['data']['chargecode'] == '00' || $response['data']['chargecode'] == '0') ){
					...
					*/
					...
					$data['verID'] = $this->session->userdata("verID");
            		...
            		$dets = $this->rss_model->getDetails($data['refID']);
